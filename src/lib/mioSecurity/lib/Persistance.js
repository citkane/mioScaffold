...
const { importPKCS8, importSPKI} = jose;
const User = require('../types/User.js');
const Service = require('../types/Service.js');
const Keygen = require('../lib/Keygen');
const { findMioRootDir } = mioUtils;

function makeSecurityStorePaths(domain){
    const rootDir = findMioRootDir(__dirname);
...
module.exports = class Persistance extends Keygen {
    constructor(_logger, domain, config){
        ...
        switch (this.storage) {
            case 'filesystem':
                ({keys, paths} = makeSecurityStorePaths(domain))
