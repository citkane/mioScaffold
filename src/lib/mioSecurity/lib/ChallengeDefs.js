...
const { 'uglify-es':uglify, checksum, crypto } = require('@mio/mioscaffold');
...
let logger;
module.exports = class ChallengeDefs {
...
new Promise((resolve, reject) => {
    ...
    Promise.all([
        ...
        hashElement(path.join(dir, 'node_modules/@mio-lib/miosecurity/src')).then(h => h.hash),
        //hashElement(path.join(dir, 'node_modules/@mio-lib/miosecurity/node_modules/mqtt/dist')).then(h => h.hash)
    ]).then(hashes => {
